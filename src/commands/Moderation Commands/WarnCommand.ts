...
export default class WarnCommand extends Command {
    ...
    public async exec(message: Message, {member, reason}: {member: GuildMember, reason: string}): Promise<Message> {
        ...
        if(member.roles.highest.position >= message.member.roles.highest.position && message.id !== message.guild.ownerID)
            return message.util.reply("This member has a higher or equal roles to you!");
